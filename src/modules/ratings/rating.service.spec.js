...
describe('RatingService', () => {
    ...
    describe('createRating', () => {
        ...
        //     const ratingData = { userId: 'userId123', movieId: 'movieId123', value: 4 };
        //     Rating.findOne.mockResolvedValue(null);
        //     Rating.mockImplementation(() => ({
        //         save: jest.fn().mockResolvedValue(ratingData)
        //     }));

        //     await RatingService.createRating(ratingData);

        //     expect(Rating.findOne).toHaveBeenCalledWith({ userId: 'userId123', movieId: 'movieId123', isDeleted: false });
        //     expect(Rating).toHaveBeenCalledWith(ratingData);
        //     expect(Movie.findByIdAndUpdate).toHaveBeenCalledWith('movieId123', expect.any(Object));
        // });

        // it('should update an existing rating if it exists', async () => {
        //     const ratingData = { userId: 'userId123', movieId: 'movieId123', value: 5 };
        //     const existingRating = { value: 3, save: jest.fn().mockResolvedValue(ratingData) };
        //     Rating.findOne.mockResolvedValue(existingRating);

        //     await RatingService.createRating(ratingData);

        //     expect(existingRating.value).toBe(5);
        //     expect(existingRating.save).toHaveBeenCalled();
        //     expect(Movie.findByIdAndUpdate).toHaveBeenCalledWith('movieId123', expect.any(Object));
        // });

        it('should throw an ApiError if there is an error during rating creation or update', async () => {
            const ratingData = { userId: 'userId123', movieId: 'movieId123', value: 4 };
            Rating.findOne.mockRejectedValue(new Error('Database error'));

            await expect(RatingService.createRating(ratingData))
        });
    });
